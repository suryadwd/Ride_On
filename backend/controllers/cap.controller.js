...
//     const { firstname, lastname, email, password, color, plate, capacity, vehicleType,vehicle } = req.body;

//     if(!firstname  || !email  || !vehicle)
...
export const register = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ success: true, message: "Captain registered successfully", newCap });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
};
